...
function formatTweakResult(tweakId, result) {
  ...
  if (tweakId === 'disable-unnecessary-services') {
    const rows = (result.results || [])
      .map((r) => `<li><svg class="inline-icon ${r.success ? 'icon-ok' : 'icon-fail'}" data-tooltip="${r.success ? 'Aplicado com sucesso' : 'Falhou ao aplicar'}"><use href="#${r.success ? 'icon-check' : 'icon-cross'}"/></svg> ${r.name}${r.error ? ` — <span class="text-muted">${r.error}</span>` : ''}</li>`)
      .join('');
...
async function loadWindowsTweaks() {
  ...
  const banner = isWindows
    ? ''
    : `<div class="presentmon-banner"><span class="presentmon-banner-text"><svg class="inline-icon" data-tooltip="Aviso"><use href="#icon-warning"/></svg> Este ambiente não é Windows — os ajustes reais (serviços, cache, prioridade) serão simulados e nada será alterado de verdade.</span></div>`;
